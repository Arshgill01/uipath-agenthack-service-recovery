...
| Confirm `uip` CLI availability. | `docs/validation/VALIDATION_RESULTS.md`; `docs/logs/BUILD_LOG.md`; `AGENTS.md`. | PASS | `uip` currently reports version `1.195.1` and is used for platform lifecycle/readback. |
| G-001 Native Case State / Audit Reconstruction. | `docs/validation/VALIDATION_GATES.md`; `docs/validation/VALIDATION_RESULTS.md`; `docs/architecture/DATA_MODEL.md`; Data Fabric E-004 row readback; Orchestrator audit bundle artifacts. | PARTIAL natively / PASS with Orchestrator full-payload custom audit storage | Native Case alone is not enough. Data Fabric CSV import is partial because row readback works but custom payload fields did not appear through CLI readback. The Orchestrator `service-recovery-audit-v1` bundle remains the validated full-payload fallback. |
...
- Claim Data Fabric only as partial row persistence for the E-004 CSV import path until custom payload fields can be read back; direct JSON insert remains unvalidated.
