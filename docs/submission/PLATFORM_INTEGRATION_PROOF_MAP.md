...
9. Optional external evidence-source proof uses `fixtures/external_evidence_CASE_BG_CONTRA.csv` as a live-style systems-of-record simulator for CRM, billing, inventory, network telemetry, and dispatch. It proves the contradiction path can be sourced from a tabular external evidence source without claiming production telecom OSS/BSS integration.
10. `scripts/run_submission_check.sh` verifies the local proof set without starting live UiPath cases, completing live tasks, or calling Gemini/Vertex.
...
| External E-004 source proof | Same contradiction pattern, sourced from a tabular external evidence simulator. | `closure_candidate` | `require_human_review`, route `human_review`, reason `source_contradiction` | External source proof is local/read-only unless a public URL is intentionally provided for a fresh run. | `docs/demo/artifacts/external_evidence_source_proof.json`, `docs/demo/artifacts/evidence_packet_external_E004.html`, `docs/demo/artifacts/service_recovery_audit_bundle_external_E004.json` | This is not real telecom production integration; it is an external systems-of-record simulator. |
...
| `docs/demo/artifacts/demo_proof_manifest.json` | Machine-readable index of the generated E-002/E-004 payload, audit, packet, recommendation, decision, block reason, and route. | Generated by `python -m service_recovery_core.demo_proof`. |
| `docs/demo/artifacts/proof_index.html` | Single judge-facing index linking E-002, E-004, E-003 adversarial, and E-008 learning-loop proof with visible claim boundaries. | Generated from existing proof artifacts by `python -m service_recovery_core.proof_index`; support surface only, not a replacement for UiPath runtime proof. |
| `docs/demo/artifacts/external_evidence_source_proof.json` | Optional proof that the E-004 contradiction path can be generated from a live-style external CRM/billing/inventory/network/dispatch evidence table. | Generated by `python -m service_recovery_core.external_evidence`; not a production telecom OSS/BSS integration. |
