...
| Test Manager automation discovery diagnostics | PF-024 | UX / integration / diagnostics | medium | If automated execution is supported, Test Manager should list available automations, return an empty list, or explain missing runtime/package prerequisites per folder. | CLI supports `link-automation` and `run --execution-type automated`; Shared folder automation discovery returned an empty list, while Solution-folder `list-automations` returned HTTP 400 `Internal Server Error` both unfiltered and filtered by package name. | Keep G-007 as terminal manual Test Manager validation only; do not claim automated Test Cloud execution until a real test automation package/target is available and linkable. | Return structured diagnostics: no automations in folder, unsupported folder type, missing Test Automation runtime, package feed mismatch, or internal correlation ID. | `docs/validation/TEST_MANAGER_MAPPING.md` 2026-06-26 16:33 UTC; PF-024. |
