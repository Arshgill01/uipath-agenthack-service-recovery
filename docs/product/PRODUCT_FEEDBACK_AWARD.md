...
| Action Center generated app task rendering and return | PF-013 | UX / product defect candidate / integration | high for G-003 legibility | Action Center should render generated app fields with the schema display names and return reviewer action/comment as structured case data. | Action Center mechanics proved claim/approve/reject/structured return, but `PolicyDecisionJson` rendered as `Unnamed String 1` while the task API showed the correct persisted value; downloaded app metadata showed the schema input exists while the generated form model has an `UnnamedString1` control. A follow-up `uip codedapp pull` against the Studio project ID failed because the generated app is not a supported coded-app source project. Safari Studio Web later exposed the broken label as `Label4.Text = "Unnamed String 1:"`; a label-only repair was published as app version `1.0.1` and Studio preview showed `Policy Decision Json:`. A fresh runtime task `4333536` after that publish still rendered `Unnamed String 1:` / `Unnamed string 1`; read-only registry/task probes showed the task runtime used older deployment system name `ID5c3f0a11590d4fdab3c22de72f4ff443` / `SemVersion: 1.0.0` while a separate `SemVersion: 1.0.1` deployment existed in another solution folder. | Use Action Center mechanics for proof of task lifecycle; use a custom packet view for demo legibility. Do not rely on label-only Studio Web repair for Case-bound runtime tasks unless a deeper binding/version propagation route is found and revalidated. | Preserve schema display names in generated runtime tasks and expose a field-binding inspector for Action Center tasks that maps schema field -> generated control -> runtime visibility; show which app version a Case task will actually instantiate; warn when a published designer change does not affect existing Case package/task bindings; expose whether a generated app is editable via Studio UI only, coded-app CLI, or neither. | `docs/validation/artifacts/2026-06-25/g003-action-center-task-renders-unassigned-fields-null.png`; `docs/validation/artifacts/2026-06-25/g004-action-center-policy-field-mislabeled-task-4295299.png`; `docs/validation/artifacts/2026-06-25/g003-action-center-e004-completed-generated-ui-empty-fields.png`; `docs/validation/artifacts/2026-06-27/product_feedback_action_binding_probe.md`; task `4333536`; case `9eb64f9f-6613-48f7-b452-215085d8c67b`; `docs/validation/ACTION_CENTER_UI_REPAIR_ASSESSMENT.md`; PF-013. |
...
| Data Fabric CLI discovery mismatch | PF-018 | UX / integration / diagnostics | medium | CLI tool discovery should show the Data Fabric capability when `uip df` is installed and usable, or explain why it is a built-in command rather than a listed tool. | `uip df --help` and `uip df entities list --native-only --output json` worked on 2026-06-25, but `uip tools list --output json` did not expose Data Fabric then. A 2026-06-27 recheck on CLI `1.195.1` did list `data-fabric-tool`, so this appears improved/resolved for current CLI discovery. | Use direct `uip df` commands and document the exact entity schema/record commands before mutating the tenant. | Keep Data Fabric discoverable in `uip tools list`; for installed built-in command surfaces, include version and command prefix consistently. | `docs/validation/VALIDATION_RESULTS.md` 2026-06-25 21:01 IST; `docs/validation/artifacts/2026-06-27/data_fabric_readback_diagnostics_probe.md`; PF-018. |
| Data Fabric record insertion/readback payload mapping | PF-019 | product defect candidate / integration / diagnostics | high for G-001 custom audit storage | After creating and reading a Data Fabric entity, `records insert`/CSV import should persist values for required custom fields and `records get/query` should return those values or explain why projection is unavailable. | Entity create/readback succeeded, but `records insert` rejected multiple snake_case JSON shapes with `case_id` reported missing. CSV import later created record `DA42769C-33B7-4701-A266-019F032AF376`, yet 2026-06-27 readback still returned only system fields even when selecting `case_id`, policy versions, and payload fields. PascalCase V2 fields still worked in the same probe. | Use `ServiceRecoveryAuditBundleV2` with PascalCase fields for the validated full-payload Data Fabric path; keep Orchestrator bucket as alternate fallback; do not use the legacy snake_case entity for final audit reconstruction. | Add schema-aware insert/import validation that echoes recognized/unrecognized fields and values, documents field-naming constraints if any, includes a correlation/session ID for support, and documents how to read back imported custom fields. | `docs/validation/VALIDATION_RESULTS.md` 2026-06-25 21:08 IST, 2026-06-26 16:38 IST, 2026-06-26 15:38 UTC, and 2026-06-27; `docs/validation/artifacts/2026-06-27/data_fabric_readback_diagnostics_probe.md`; PF-019. |
| Data Fabric custom-field naming and false-success updates | PF-023 | product defect candidate / integration / diagnostics | high for Data Fabric adoption | Valid custom field names allowed by entity creation should behave consistently for insert, update, query, and get, or the platform should reject unsupported names at schema creation. | snake_case fields were accepted in entity schemas, but JSON insert/update did not persist them; update returned `RecordUpdated` even though query/get showed no custom value. PascalCase fields in `DataFabricPascalProbe` and `ServiceRecoveryAuditBundleV2` worked. On 2026-06-27, a wrong-case V2 query failed clearly with `Filter field case_id does not exist`, which is useful read-time validation, but the write-time help still did not explain the snake_case/PascalCase boundary. | Use PascalCase field names for current Data Fabric audit entities and verify with query/get after every write. | Validate custom field names across the whole lifecycle, return write results that include accepted and ignored fields, make false-success updates impossible, and document any naming restrictions in CLI and UI schema builders. | `docs/validation/VALIDATION_RESULTS.md` 2026-06-26 15:38 UTC; `docs/validation/artifacts/2026-06-27/data_fabric_readback_diagnostics_probe.md`; PF-023. |
| Test Manager eval-suite onboarding | PF-020 | UX / integration | medium | A local eval output can be imported or mapped into Test Manager without bespoke object creation. | CLI project/case/test-set lifecycle worked, but E-001 through E-009 had to be manually represented as Test Manager objects. A 2026-06-27 read-only probe found CLI result download/export surfaces and official UI imports for manual Excel cases / Orchestrator test sets, but no direct local eval JSON/JUnit import path into Test Manager cases/test sets. | Created project `SREV`, nine manual cases, test set `SREV:9`, and mapping doc; keep local eval output as source of truth and Test Manager mapping as manual representation. | Add eval/JUnit/JSON import flow with scenario preview, field mapping for AI evals, labels, expected/actual policy fields, and artifact attachment. | `docs/validation/TEST_MANAGER_MAPPING.md`; `docs/validation/artifacts/2026-06-27/pf-workstream-c/README.md`; PF-020. |
| Test Manager manual execution aggregate status | PF-021 | UX / product defect candidate | medium | Manual execution aggregate becomes terminal or explains the required close action when all child logs pass. | The first run marked all nine child logs passed through direct `finish` calls, but the aggregate stayed `Running`; a later explicit `start` then `finish` lifecycle reached terminal `Status: Finished`. A 2026-06-27 read-only probe still showed the older direct-finish execution as `Running` with `Passed: 9`, while the terminal execution remained `Finished` with `Passed: 9`. | Use `testcaselog start` before `testcaselog finish`; cite the terminal execution and keep the first run as lifecycle ambiguity evidence. | Add explicit finish/close manual execution action, warn when finishing a never-started manual log, or surface the remaining blocker in execution readback. | `docs/validation/TEST_MANAGER_MAPPING.md`; terminal execution `40a1b334-5df8-1100-0a4b-0b49d0564f11`; `docs/validation/artifacts/2026-06-27/pf-workstream-c/README.md`; PF-021. |
| Case job/task lifecycle CLI clarity | PF-022 | UX / integration / diagnostics | medium | Process, job, task, and Case Instance readbacks should make a fresh Case run's package, human-task completion, and terminal case state easy to reconstruct with consistent flags. | `processes get` rejected `--folder-key` while `version-history` accepted it; completed E-002/E-004 AppTasks returned reviewer actions/comments, but the first readback pass still showed older Case jobs as `Running` with only Pending/Running history. Fresh Wave 42 readback later showed those E-002/E-004 case instances completed, and fresh package `1.0.6` Case Instances also reached `LatestRunStatus: Completed`. | Use corrected `processes get <key>` command, read `version-history`, rely on task readback plus audit bundle for reviewer proof, and use Data Fabric/custom audit proof for the domain decision trail. Native completion status is useful lifecycle evidence but not full domain audit proof. | Harmonize folder option support or document per-subcommand differences; show whether a Case job is waiting for human task, post-task continuation, optional placeholder task, terminal completion, or blocked state. | `docs/validation/VALIDATION_RESULTS.md` 2026-06-26 00:36 IST, 2026-06-26 16:00 IST, and 2026-06-29 Wave 42; `docs/demo/DEMO_SAFE_PROOF_RUNBOOK.md`; PF-022. |
...
| Scratch Case human-review authoring preflight consistency | PF-028 | UX / diagnostics / integration / product defect candidate | high for new-builder human-review workflow | A scratch Case authoring path should scaffold a complete minimal case or guide the builder to create missing content, name required Action task fields precisely, and block/flag invalid Case definitions consistently in solution dry-run/upload. | `case init` created project metadata whose entry point referenced absent `content/caseplan.json`; after creating a scratch case/stage, `tasks add` allowed an Action task without `--task-title`; `case validate` caught a generic empty required field but did not name `Title`; `tasks update` exposed no title repair flag; `solution pack --dry-run` returned `Status: Valid`; and `solution upload` created scratch Studio Web solution `PFPROBE-20260627-human-review` with `ErrorList: []`. A follow-up repair probe showed the only CLI repair path found was remove/re-add with `--task-title`; after adding Title, required task, and rules, `case validate` passed and Studio Web upload/download preserved validity. | Use `tasks describe --id <action-app-id>` to inspect schema, run `uip maestro case validate` manually, and treat solution dry-run/upload success as insufficient for Case human-review readiness. For missing title, remove/re-add the task with `--task-title` if no in-place update path is available. Do not modify existing submission assets. | Make Case validate, solution dry-run, and Studio Web upload/import share the same readiness checks for Case rules, Action task required fields, Action app binding/version, and repair commands; name missing fields such as `Title` and expose an update path or Studio deep link. | `docs/validation/artifacts/2026-06-27/product_feedback_phase2_scratch_case_probe.md`; `docs/validation/artifacts/2026-06-27/product_feedback_workstream_a_maestro_authoring_repair_probe.md`; PF-028. |
| Integration Service connector readiness for optional external evidence | PF-029 | UX / docs / integration | medium | When a builder wants to use an external evidence source, the platform should show whether the connector exists, a connection is configured, OAuth is complete, activities/resources are available, and docs/deep links are usable. | `uip is connectors list`, `uip is activities list`, and `uip is resources list` showed relevant connectors and activities for Google Sheets, HTTP, Data Fabric, Orchestrator, Gmail, Slack, ServiceNow, Jira, and Salesforce, but `uip is connections list` returned no connections. Connector detail for Google Sheets/HTTP showed no ready documentation URL, and catalog rows reported inactive states. | Keep the optional external-evidence proof as a public CSV/simulator path; do not claim real Integration Service integration until a connection is configured and exercised. | Add connector readiness diagnostics and fix links to the same Maestro human-review preflight: connector exists, connection configured, OAuth complete, docs/deep link available, and required activity/resource present. | `docs/validation/artifacts/2026-06-29/wave42/integration_data_test_probe.md`; PF-029. |
| Test Manager automation discovery diagnostics | PF-024 | UX / integration / diagnostics | medium/high | If automated execution is supported, Test Manager should list available automations, return an empty list, or explain missing runtime/package prerequisites per folder. Automated test-set runs should identify whether the blocker is folder binding, unlinked manual cases, missing package, runtime type, package metadata, or permissions. | CLI supports `link-automation` and `run --execution-type automated`; Shared folder automation discovery returned an empty list, while Solution/personal folder `list-automations` returned HTTP 400 `Internal Server Error`. Automated `SREV:9` first failed because no folder was assigned, setting project default to Solution failed HTTP 500, setting default to Shared succeeded, then automated execution failed with `No Automatic package selection could be done for test set to execute.` Follow-up probe built and uploaded package `ServiceRecoveryEvalProcessProbe:0.0.2/0.0.3`; Orchestrator listed an entry point, but Test Manager discovery still returned `Data: []`, and direct `link-automation` failed with `Test ... not found in package`. A 2026-06-27 read-only repeat showed package `ServiceRecoveryEvalProcessProbe:0.0.3` still package-visible as `PackageType: Process` / `IsActive: false`, while `list-automations` returned `Data: []` even with `--log-level debug` and no actionable package-metadata reason. | Keep G-007 as terminal manual Test Manager validation only; do not claim automated Test Cloud execution until a supported UiPath test automation project can publish a Test Manager-visible test entry point. The project default folder is currently set to Standard `Shared` because that was the only CLI folder assignment path that succeeded. | Return structured diagnostics: package has no Test Manager-visible test cases, package is process-only, test metadata was stripped/ignored during pack, no automations in folder, unsupported folder type, missing Test Automation runtime, package feed mismatch, unlinked test cases, invalid folder type, inactive package, or internal correlation ID; provide a CLI command that validates whether a package will appear in `list-automations` before upload/link attempts. | `docs/validation/TEST_MANAGER_MAPPING.md` 2026-06-26 16:33 UTC; `docs/validation/VALIDATION_RESULTS.md` 2026-06-26 14:55 UTC and 2026-06-26 15:04 UTC; `docs/validation/artifacts/2026-06-27/pf-workstream-c/README.md`; PF-024. |
...
- PF-029 adds a narrow external-evidence readiness finding: Integration Service connector catalogs and activities were discoverable, but there were no configured connections and not enough docs/deep-link guidance to treat the connector path as ready. This should support the readiness-check recommendation without becoming a real-integration claim.
...
| PF-013 | 2026-06-25 | Action Center / generated app task | Claim, approve/reject, structured return, field rendering | G-003 | UX / product defect candidate / integration | high | repeated / fallback activated | `docs/validation/artifacts/2026-06-25/g003-action-center-e004-completed-generated-ui-empty-fields.png` |
...
| PF-018 | 2026-06-25 | Data Fabric / UiPath CLI | CLI command discovery for Data Fabric | G-001 / G-002 / G-008 | UX / integration / diagnostics | medium | improved/resolved in current CLI | `docs/validation/VALIDATION_RESULTS.md` |
...
| PF-029 | 2026-06-29 | Integration Service / UiPath CLI | External evidence connector readiness | Wave 42 / optional external evidence | UX / docs / integration | medium | observed / not integrated | `docs/validation/artifacts/2026-06-29/wave42/integration_data_test_probe.md` |
...
- If an Action task title is missing and no update path is available, remove/re-add the scratch task with `--task-title`; then add stage/task/case rules and requiredness before upload.
...
- Repaired local scratch path: `tmp/product-feedback-probes/PFPROBE-20260627-human-review-repair`.
...
### PF-029 - 2026-06-29 - Integration Service / External Evidence Connector Readiness

Context:

- ID: PF-029.
- Status: observed / not integrated.
- Goal: test whether optional external evidence sources should be promoted from public CSV/simulator proof to real UiPath Integration Service proof.
- Product surface: UiPath Integration Service CLI.
- Account/tenant: `keepingitlowkey` / `DefaultTenant`, user `arshgill6120@gmail.com`.
- Wave/gate: Wave 42 / optional external evidence.

What worked:

- `uip is connectors list --output json` exposed a broad Integration Service connector catalog, including Google Sheets, HTTP, Data Fabric, Orchestrator, Gmail, Slack, ServiceNow, Jira, and Salesforce.
- `uip is activities list uipath-google-sheets --output json` exposed spreadsheet operations such as read, write, insert, update, and query-style activities.
- `uip is activities list uipath-uipath-http --output json` exposed an HTTP request activity.
- Data Fabric and Orchestrator Integration Service activities/resources were also discoverable.

What failed or confused us:

- `uip is connections list --output json` returned no configured connections.
- Connector detail for Google Sheets/HTTP did not provide a ready documentation URL in the CLI readback.
- Catalog/readback fields reported inactive states, which makes it hard for a first-time builder to know whether the missing step is catalog enablement, OAuth connection creation, tenant policy, or simply lack of configured credentials.

Expected:

- A builder should be able to run one readiness check that says: connector exists, connection configured, OAuth complete, required activity/resource available, docs/deep link available, and the current Case/Flow can use the connection.

Observed:

- The connector/activity surfaces were discoverable, but the tenant was not ready to execute a real Google Sheets/HTTP external-evidence integration because no connection was configured.
- The evidence did not justify claiming real Integration Service integration in the submission.

Impact:

- Build impact: medium. This could make the external evidence source feel more real, but only if the connector path is actually configured and exercised.
- Demo/submission impact: medium. We kept the optional external-evidence proof honest as a public CSV/simulator path and did not add an unsupported real-integration claim.
- Severity: medium.

Workaround:

- Use the existing external evidence simulator/public CSV proof for the submission.
- Mention Integration Service only as a discovered-but-not-configured readiness finding unless a real connection is later configured and a live read/write/readback is performed.

Suggested improvement:

- Add connector readiness diagnostics to the Maestro human-review readiness check: connector installed/catalog-visible, active, connection configured, OAuth complete, docs/deep link available, activity/resource present, and current workflow binding ready.
- If a connector is discoverable but not usable, show the exact next action and the admin/user surface to create or authorize the connection.
...
- high for the non-claim boundary and medium for the product improvement, because the commands were run live and read-only against the authenticated tenant, but no connector connection was configured or exercised.

Follow-up validation needed:

- Optional only. If the team wants to claim real Integration Service usage later, configure one low-risk connection and run an actual read/write/readback before updating submission claims.
...
- E-002 and E-004 AppTasks were completed with reviewer comments, but the first readback pass still showed the corresponding Case jobs as `State: Running`.
- E-004 `jobs history` only showed `Pending` and `Running` during that pass; it did not explain whether the Case was waiting on post-task continuation, long-running Case lifecycle, or another completion condition.
- Fresh Wave 42 readback later showed the E-002 and E-004 case instances completed. This updates the completion boundary but does not turn native completion status into full domain audit proof.
...
- E-002 and E-004 jobs initially read back as `Running`; Wave 42 later read the case instances as completed.
...
- Demo/submission impact: medium. The final demo can honestly show task lifecycle and audit bundle proof. Native completion status can be cited as lifecycle evidence, but the Data Fabric/custom audit remains the domain decision proof.
- Severity: medium.
...
- ID: PF-013.
- Status: repeated / fallback activated.
...
- A read-only inspection of downloaded generated app assets found the expected `PolicyDecisionJson` schema input, while the generated form model exposed an `UnnamedString1` control instead of a readable policy field.
- A Studio Web label-only repair changed `Label4.Text` to `"Policy Decision Json:"` and published app version `1.0.1`, but a fresh Case-bound runtime task still rendered `Unnamed String 1:` / `Unnamed string 1`.
...
- Safari/Computer Use inspection of completed task `4300219` after reload showed the task in `Completed` with reviewer comment visible, but generated UI fields `Content`, `Evidence Packet Json`, and `Raw Agent Recommendation` had no visible values and the policy field still appeared as `Unnamed String 1`.
- Task `4333536`, created by fresh package `1.0.6` Case Instance `9eb64f9f-6613-48f7-b452-215085d8c67b` after the Studio Web publish, persisted correct `PolicyDecisionJson` through `uip tasks get`.
- Safari/Computer Use inspection of pending task `4333536` still showed `Unnamed String 1:` with value `Unnamed string 1`, so the label-only Studio publish did not repair the Case-bound runtime page.
- Reviewer lifecycle worked after assignment; task `4333536` was completed with `reject`, and the case later reached `LatestRunStatus: Completed`.
- A 2026-06-27 read-only binding probe found two `SimpleApprovalApp` deployments in the Case registry: `SemVersion: 1.0.1` in folder `Solution 1` with system name `IDb707cd2abbdd42178b415f7341a65f13`, and `SemVersion: 1.0.0` in folder `Solution` with system name `ID5c3f0a11590d4fdab3c22de72f4ff443`.
- Runtime task `4333536` reported `AppTasksMetadata.AppId: ID5c3f0a11590d4fdab3c22de72f4ff443` and `AppVersion: 1`, matching the older `SemVersion: 1.0.0` deployment; the process readback exposed package version `1.0.6` but not the Action app deployment/version it would instantiate before runtime.
- `tasks describe` exposed the schema fields but left `ElementId` empty and did not show generated page controls or labels, so the current CLI/API path is not a complete field-binding inspector.
...
- For demo polish, use the custom evidence-packet view. Do not depend on label-only Studio Web repair unless the Case package/task binding route is understood and a fresh task proves the rendered label and value.
...
- Show the exact generated app version and package binding used by each Case/AppTask, and warn when a Studio Web publish does not affect the Case-bound runtime task.
...
- Screenshot/path/link: `docs/validation/artifacts/2026-06-25/g003-action-center-e004-completed-generated-ui-empty-fields.png`.
- Evidence artifact: `docs/validation/artifacts/2026-06-27/product_feedback_action_binding_probe.md`.
- Live task/case: task `4333536`, case `9eb64f9f-6613-48f7-b452-215085d8c67b`, observed in Safari Action Center on 2026-06-26 after Studio Web publish.
...
- A 2026-06-27 read-only recheck confirmed the split remains current: legacy record `DA42769C-33B7-4701-A266-019F032AF376` still returns only system fields, while V2 record `F9D838CE-4671-F111-AC9A-0022489A9A06` still returns first-class fields and full JSON payloads.
...
- Follow-up CSV import created/read back record `DA42769C-33B7-4701-A266-019F032AF376` by `Id` after using the Data-Fabric-safe wire format, but custom field values were not visible through CLI readback.
- Current CLI help explains JSON object/file input shapes for insert/update/query, and `entities get` exposes field metadata, but the help does not explain why schema-valid snake_case fields fail write/readback while PascalCase fields work.
...
- Commands/logs: `docs/validation/artifacts/2026-06-27/data_fabric_readback_diagnostics_probe.md`.
...
- A 2026-06-27 V2 wrong-case query using `case_id` against `ServiceRecoveryAuditBundleV2` failed clearly with `Filter field case_id does not exist or is deleted`, which is useful read-time validation. The missing product gap is still write-time/update-time validation for schema-valid names that later do not persist/read correctly.
...
- `uip tools list --output json` now lists `data-fabric-tool` on CLI `1.195.1`; the current issue is less CLI discovery and more lifecycle diagnostics for accepted field names and record projections.
