...
| Action Center generated app task rendering and return | PF-013 | UX / product defect candidate / integration | high for G-003 legibility | Action Center should render generated app fields with the schema display names and return reviewer action/comment as structured case data. | Action Center mechanics proved claim/approve/reject/structured return, but `PolicyDecisionJson` rendered as `Unnamed String 1` while the task API showed the correct persisted value; downloaded app metadata showed the schema input exists while the generated form model has an `UnnamedString1` control. A follow-up `uip codedapp pull` against the Studio project ID failed because the generated app is not a supported coded-app source project. Safari Studio Web later exposed the broken label as `Label4.Text = "Unnamed String 1:"`; a label-only repair was published as app version `1.0.1` and Studio preview now shows `Policy Decision Json:`. A fresh Action Center task has not yet revalidated runtime rendering. | Use Action Center mechanics for proof of task lifecycle; use a custom packet view for demo legibility unless the republished Studio Web label repair is revalidated with a fresh task. | Preserve schema display names in generated runtime tasks and expose a field-binding inspector for Action Center tasks that maps schema field -> generated control -> runtime visibility; expose whether a generated app is editable via Studio UI only, coded-app CLI, or neither; make publish/deploy/fresh-task revalidation explicit after generated field repair. | `docs/validation/artifacts/2026-06-25/g003-action-center-task-renders-unassigned-fields-null.png`; `docs/validation/artifacts/2026-06-25/g004-action-center-policy-field-mislabeled-task-4295299.png`; `docs/validation/artifacts/2026-06-25/g003-action-center-e004-completed-generated-ui-empty-fields.png`; `docs/validation/ACTION_CENTER_UI_REPAIR_ASSESSMENT.md`; PF-013. |
