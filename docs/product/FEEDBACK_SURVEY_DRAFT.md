...
| Data Fabric record insert/readback needs schema-aware diagnostics. | A record body keyed by schema field names should insert or explain exact expected shape, and imported custom fields should be readable through get/query. | Entity create/readback succeeded, but snake_case direct JSON insert shapes failed with required `case_id` reported missing; CSV import created rows that read back only system fields. PascalCase V2 fields later inserted/query/read back correctly. | Use `ServiceRecoveryAuditBundleV2` with PascalCase fields for the full-payload audit proof; keep the legacy snake_case entity as feedback evidence. | Echo recognized/unrecognized fields, document field-naming constraints if any, and provide insert/import/readback examples for custom required fields and nested payload text fields. |
...
| Manual Test Manager execution status needs clearer terminal behavior. | If every manual test case log is passed, the aggregate execution should finish or explain the remaining close action. | Execution `d50a7be6-35ed-1100-95aa-0b49cf9b8cad` showed `Passed: 9`, `Failed: 0`, `None: 0`, but top-level `Status: Running`; a later explicit start-then-finish lifecycle produced terminal execution `40a1b334-5df8-1100-0a4b-0b49d0564f11`. | Use `testcaselog start` before `finish`; cite the terminal execution and JUnit export. | Add explicit finish-manual-execution action or status blocker details; warn/autostart when finishing a case log that has not been started; make wait polling interval compatible with short timeouts. |

### Do Not Claim Yet

- Do not claim automated Test Cloud execution. A live Test Manager project/test set and terminal manual execution now exist, but no automated run/link has been validated.
- Do not claim the legacy snake_case Data Fabric entity as the audit proof. Use the validated PascalCase `ServiceRecoveryAuditBundleV2` path for queryable Data Fabric audit readback.
