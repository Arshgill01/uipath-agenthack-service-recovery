...
| Data Fabric record insert/readback needs schema-aware diagnostics. | A record body keyed by schema field names should insert or explain exact expected shape, and imported custom fields should be readable through get/query. | Entity create/readback succeeded, but direct JSON insert shapes failed with required `case_id` reported missing. CSV import created an E-004 row only after a custom nested-payload wire format, but follow-up CLI readback returned only system fields. | Use Orchestrator bucket artifact as the full-payload audit proof; treat Data Fabric as partial row persistence. | Echo recognized/unrecognized fields and provide insert/import/readback examples for custom required fields and nested payload text fields. |
