...
| Data Fabric record insert needs schema-aware diagnostics. | A record body keyed by schema field names should insert or explain exact expected shape. | Entity create/readback succeeded, but direct JSON insert shapes failed with required `case_id` reported missing. CSV import worked only after a custom nested-payload wire format. | Use the validated E-004 CSV import path or Orchestrator bucket artifact. | Echo recognized/unrecognized fields and provide insert/import examples for custom required fields and nested payload text fields. |
...
- Do not claim broad Data Fabric audit storage; the validated path is E-004 CSV import using the Data-Fabric-safe wire format, while direct JSON insert remains unvalidated.
